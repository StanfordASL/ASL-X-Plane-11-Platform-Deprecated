...
def main():
    with xpc3.XPlaneConnect() as client:
        record(client, settings.OUT_DIR, startPerc=settings.START_PERC + 1.0, endPerc = settings.END_PERC - 1.0, 
                    freq=settings.FREQUENCY, numEpisodes=settings.NUM_TRAJECTORIES)
...
def addCurrData(client, outDir, csvFile, startTime, currStep, episodeNum):
    ...
    weather = settings.CLOUD_TYPES[cloud_cover]
